...
for fold_idx, (train_index, test_index) in enumerate(kf.split(X), 1):
    ...
    model = create_bi_lstm_model(input_shape, num_classes)
    ...
    plt.title(f"Bi-LSTM - Fold {fold_idx} Confusion Matrix")
    plt.xlabel("Predicted")
    plt.ylabel("True")
    plt.savefig(f"confusion_matrices/bi_lstm_fold_{fold_idx}.png")
    plt.close()

# Save results
timing_df = pd.DataFrame(timing_results)
timing_df.to_csv("bi_lstm_time.csv", index=False)

results_df = pd.DataFrame(results)
results_df.to_csv("bi_lstm_metrics.csv", index=False)
print(results_df)
